...
class AdminController extends Adminaddonbase {
    ...
    public function index() {
        if (IS_POST) {
            ...
            //栏目模型
            $CategoryModel = D("Content/Category");
            //开始操作
                    ...
                    switch ($action) {
                        ...
                        case 'listgenerate':
                            ...
                            if ($info['setting']['ishtml']) {
                                $info['setting']['category_ruleid'] = I('post.category_html_ruleid', 0, 'intval');
                            } else {
                                $info['setting']['category_ruleid'] = I('post.category_php_ruleid', 0, 'intval');
                            }
                            break;
                        // 内容页生成Html
                        case 'showgenerate':
                            ...
                            if ($info['setting']['content_ishtml']) {
                                $info['setting']['show_ruleid'] = I('post.show_html_ruleid', 0, 'intval');
                            } else {
                                $info['setting']['show_ruleid'] = I('post.show_php_ruleid', 0, 'intval');
                            }
                            break;
                        // 后台角色权限
                        case 'adminrole':
                            D("Content/Category_priv")->update_priv($catid, $_POST['priv_roleid']);
                            break;
                        // 前台用户组权限
                        case 'memberrole':
